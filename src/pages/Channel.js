...
function Channel() {
  ...
  const fetchUserProfile = async()=>{
    try {
      const response = await fetch(process.env.API_ENTRYPOINT+"/api/v1/users/c/"+userId,{
        method:"GET",
        credentials:"include",
        headers: {
  ...
  const toggleSubscribe=async ()=>{
    try {
      const response = await fetch(process.env.API_ENTRYPOINT+"/api/v1/subscriptions/c/"+userId,{
        method:"POST",
        credentials:"include",
        headers: {
          "Content-Type": "application/json",
